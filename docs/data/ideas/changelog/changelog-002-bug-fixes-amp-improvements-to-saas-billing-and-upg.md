...
-   When a subscription with a special price went into past\_due or cancelled state and the sub-account attempted reactivation, the payment modal showed an incorrect amount.
-   Fix: The reactivation flow now correctly displays the actual special price amount.

**Upgrade Functionality Not Working for Some Sub-Accounts**
...
-   A bug prevented this setting from propagating to sub-accounts. Even when agencies enabled upgrades, sub-accounts still could not upgrade.
